...
async def client(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if client_exists(valida):
        await update.message.reply_text(" El cliente ya esta cargado.")
        return ConversationHandler.END
    
    context.user_data["client"] = valida

    create_client(nombre=context.user_data["client"])
    
    context.user_data.clear()

    await update.message.reply_text(" Cliente cargado con exito.")
    return ConversationHandler.END
